...
<body>
	...
	<div class="page-container">
		<form action="Book_listDeal" method="get">
			...
			<div class="text-c">
				名称： <input type="text" class="input-text" style="width: 250px"
					placeholder="输入书籍名称" id="bookName" name="bookName"
					value="${categoryName}">
				<button type="submit" class="btn btn-success radius" id="" name="">
					<i class="Hui-iconfont">&#xe665;</i> 搜书籍名称
				</button>
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a href="javascript:;" onclick="datadel()"
				...
				onclick="item_add('添加书籍','Book_insert','800','510')"
				class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i>
					添加书籍</a></span> <span class="r">共有数据：<strong>${pagerItem.rowCount}</strong>
				条
			</span>
		</div>
		<div class="mt-20">
			<table
				...
				<thead>
					<tr class="text-c">
						...
						<th width="80">书籍ID</th>
						<th width="80">书籍类别名称</th>
						<th width="80">书籍名称</th>
						<th width="80">书籍出版社</th>
						<th width="80">书籍作者</th>
						<th width="80">书籍简介</th>
						<th width="80">书籍价格</th>
						<th width="80">书籍库存数</th>
						<th width="80">状态</th>
						<th width="80">操作</th>
					</tr>
				</thead>
				...
				<tr class="text-c change" id="${item.bookId}">
					<td><input type="checkbox" value="${item.bookId}" name=""></td>
					<td>${item.bookId}</td>
					<c:choose>
					  <c:when test="${item.bookCategory!=null}">
					  	<td id="categoryParentIdName">${item.bookCategory.categoryName}</td>
					  </c:when>
					   <c:otherwise>
					   		<td></td>
					   </c:otherwise>
					...
					<td>${item.bookNum}</td>
					<c:choose>
					  <c:when test="${item.enableStatus==1}">
					  	<td><span class="label label-success radius" id="${item.bookId}1" onclick="changeStatus(${item.bookId}+'1')">下架ing</span></td>
					  </c:when>
					    <c:when test="${item.enableStatus==2}">
					  	<td><span class="label label-success radius" id="${item.bookId}2" onclick="changeStatus(${item.bookId}+'2')">上架ing</span></td>
					  </c:when>
					   <c:otherwise>
					   		<td><span class="label label-success radius" id="${item.bookId}0" onclick="changeStatus(${item.bookId}+'0')">未安排ing</span></td>
					   </c:otherwise>
					</c:choose>
					<td class="td-manage">
					<a style="text-decoration: none"
						onClick="item_del(this,${item.bookId})" href="javascript:;" title="删除" class="ml-5">
						<i class="Hui-iconfont">&#xe6e2;</i></a> 
					<a title="编辑"
						href="javascript:;"
						onclick="item_detail('编辑[id=${item.bookId}]','Book_update?id=${item.bookId}','1','800','500')"
						class="ml-5" style="text-decoration: none"><i
							class="Hui-iconfont">&#xe6df;</i></a> 
						<a title="查看" href="javascript:;" 
						onclick="item_detail('查看[Id=${item.bookId}]','Book_detail?id=${item.bookId}','4','800','500')"
						class="ml-5" style="text-decoration: none"><i
							class="Hui-iconfont">&#xe707;</i></a>
					</td>
				</tr>
	...
	<script type="text/javascript">
		...
		{
			var vid = id;
			var statusText = $("#"+id).text();
			if(statusText=="上架ing")
				id+='c';
			else if(statusText="下架ing")
				id+='b';
			else
				id+='a'
			//alert(id);
